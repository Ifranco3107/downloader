...
object Settings {
    ...
    fun init(ctx: Context?) {
        try {
           ...
           // println("Settings - init")
            ...
            println("Settings.init - ERROR: "+ ex.message)
   ...
   // @JvmStatic
    ...
    fun getSetting(key: String, valDefault: String): String {
        init(context)
        var result: String = valDefault
        try {
            result = sharedPref!!.getString(key, valDefault).toString()
        } catch (ex: Exception) {
            println("String getSetting- key: ["+key+"]")
            if(ex.message!= null)
                ErrorMgr.guardar(TAG, "String getSetting ["+key+"]", ex.message)
        }
        return result
    }
