...
/*        if (!isVisibleOrLegacy(node)) {
    ...
    private static boolean isSpeakingNode(Context context, AccessibilityNodeInfoCompat node) {
        if (hasText(node)) {
            //IFA    LogUtils.log(AccessibilityNodeInfoUtils.class, Log.VERBOSE,
            //IFA          "Speaking, has text");
            return true;
        }
        ...
        if (node.isCheckable()) {
            //IFA  LogUtils.log(AccessibilityNodeInfoUtils.class, Log.VERBOSE,
            //IFA           "Speaking, is checkable");
            return true;
        }

        // Special case for web content.
        //IFA     if (WebInterfaceUtils.hasWebContent(node)) {
           //IFA LogUtils.log(AccessibilityNodeInfoUtils.class, Log.VERBOSE,
            //IFA         "Speaking, has web content");
        //IFA         return true;
        //IFA     }
        ...
        if (hasNonActionableSpeakingChildren(context, node)) {
            //IFA LogUtils.log(AccessibilityNodeInfoUtils.class, Log.VERBOSE,
            //IFA        "Speaking, has non-actionable speaking children");
            return true;
        }
    ...
    private static boolean hasNonActionableSpeakingChildren(
        ...
        for (int i = 0; i < childCount; i++) {
            try {
                ...
                if (child == null) {
                    //IFA    LogUtils.log(AccessibilityNodeInfoUtils.class, Log.VERBOSE,
                    //IFA            "Child %d is null, skipping it", i);
                    continue;
                }
                ...
                if (!isVisibleOrLegacy(child)) {
                    //IFA      LogUtils.log(AccessibilityNodeInfoUtils.class, Log.VERBOSE,
                    //IFA            "Child %d is invisible, skipping it", i);
                    continue;
                }

                // Ignore focusable nodes.
                //IFA     if (FILTER_ACCESSIBILITY_FOCUSABLE.accept(context, child)) {
                //IFA     LogUtils.log(AccessibilityNodeInfoUtils.class, Log.VERBOSE,
                //IFA             "Child %d is focusable, skipping it", i);
                //IFA    continue;
                //IFA    }
//IFA
                ...
                if (isSpeakingNode(context, child)) {
                    //IFA    LogUtils.log(AccessibilityNodeInfoUtils.class, Log.VERBOSE,
                    //IFA             "Does have actionable speaking children (child %d)", i);
                    return true;
                }
            } finally {
        ...
        //IFA    LogUtils.log(AccessibilityNodeInfoUtils.class, Log.VERBOSE,
        //IFA            "Does not have non-actionable speaking children");
        return false;
    }
    ...
    public static boolean isSelfOrAncestorFocused(
        ...
        if (node.isAccessibilityFocused()) {
            ...
            //IFA   return hasMatchingAncestor(context, node, new NodeFilter() {
            //IFA      @Override
            //IFA      public boolean accept(Context context, AccessibilityNodeInfoCompat node) {
            //IFA          return node.isAccessibilityFocused();
            //IFA      }
            //IFA });
            return false;
        }
